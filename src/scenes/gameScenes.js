...
const scenes = {
	...
	// Используем функцию-условие без вложенного thread
	when(
	    game => game.inventory && game.inventory.includes('Золотая монета'),
	    // Просто применяем функции к текущей сцене, без создания новой
	    withConsumingChoice('Положить монету на алтарь', 'altar_activated', 'Золотая монета')
	),
	// Добавляем звуковой эффект независимо от условия
	withSoundChoice('Положить монету на алтарь (со звуком)', 'altar_activated', 'danger')
    ),
